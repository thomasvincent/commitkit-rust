...
impl ChangelogManager {
    ...
    pub fn add_entry(&self, commit_type: &str, scope: Option<&str>, subject: &str, body: Option<&str>) -> Result<()> {
        ...
        if insert_index > 0 {
            ...
            lines.push(format!("## {} ({})", version, chrono::Local::now().format("%Y-%m-%d")));
    ...
    fn format_entry(&self, commit_type: &str, scope: Option<&str>, subject: &str, body: Option<&str>) -> String {
    ...
    pub fn update_version(&self, new_version: &str) -> Result<()> {
        ...
        for i in 0..lines.len() {
            if lines[i].starts_with("## Unreleased") {
                lines[i] = format!("## {} ({})", new_version, chrono::Local::now().format("%Y-%m-%d"));
